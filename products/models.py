...
from campaigns.models import Campaign, CampaignAdditionalDetails

# import analytics.models as analytics_models
from campaigns.constants import CATEGORY_CHOICES
from .constants import CODE_POSITION_CHOICES
...
class Product(models.Model):
    ...
    brand = models.ForeignKey(
        brand_models.Brand, on_delete=models.CASCADE, db_column="brand_code"
    )
...
class ProductMarketDetails(models.Model):
    ...
    market = models.ForeignKey(
        Market, on_delete=models.CASCADE, db_column="market_name"
    )
...
class SalesData(models.Model):
    product = models.ForeignKey(
        ProductMarketDetails, on_delete=models.CASCADE, db_column="product_name"
    )
    market = models.ForeignKey(
        Market, on_delete=models.CASCADE, db_column="market_name"
    )
    ...
    def __str__(self) -> str:
        return (
            f"{self.product.product.product_name} - {self.from_date} : {self.to_date}"
        )
...
class PlanningDashboardDetails(models.Model):
    ...
    product_salesdata = models.ForeignKey(
        SalesData, on_delete=models.SET_NULL, null=True, blank=True
    )
    product_market_details = models.ForeignKey(
        ProductMarketDetails, on_delete=models.CASCADE
    )
...
class QRCodeCutomizationTemplates(models.Model):
    ...
    sample_qr_code = models.FileField(
        upload_to="products_qr_codes", null=True, blank=True
    )
