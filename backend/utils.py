...
def create_evrythng_project_application(project_id, application_name):
    ...
    if request_url:
        ...
        if access_token:
            ...
            json_data = {"name": application_name, "socialNetworks": {}}
            response = requests.post(url=request_url, headers=headers, json=json_data)
            if response.status_code == 201 and "id" in response.json():
                success = True
            else:
...
def create_evrythng_product(
    tags, brand_name, product_name, product_description, gtin, project_id
):
    """
...
#     access_token = os.environ.get("EVRYTHNG_API_ACCESS_TOKEN")
#     if request_url:
#         request_url = request_url + f"/redirections"
#         if access_token:
...
#             }
#             json_data = {"defaultRedirectUrl": default_redirect_url, "evrythngId": product_id}
#             response = requests.post(url=request_url, headers=headers, json=json_data)
#             breakpoint()
#             print(response.status_code)
...
def create_evrythng_product_redirection(product_id, default_redirect_url):
    """
    Creates a product redirection in the Evrythng platform.
    ...
    Returns:
        tuple: A tuple containing a boolean indicating success and the response from the API.
    """
    success = False
    response = {}
    ...
    if request_url and access_token:
        # Construct the request URL
        request_url = f"{request_url}/redirections"

        # Append the productId parameter to the default redirect URL
        default_redirect_url += f"?productId={product_id}"
...
def create_evrythng_application_level_redirector(project_id, application_id, rules):
    ...
    if request_url:
        request_url = (
            request_url
            + f"projects/{project_id}/applications/{application_id}/redirector"
        )
        if access_token:
            headers = {
                "content-type": "application/json",
...
def create_evrythng_account_level_redirector(
    name, project_id, product_id, application_id
):
    """
    Creates the evrythng product redirection by product id
    """
    ...
    if request_url:
        ...
        if access_token:
            rules_list = []
            existing_rules_found, existing_rules = (
                get_existing_evrythng_account_level_redirector()
            )
                ...
                [
                    {
                        ...
                        "name": name,
                        "delegates": [{"app": application_id, "project": project_id}],
                    }
                ]
            )
            headers = {
...
def generate_evrythng_qr_code(gtin, product, tpl="default", width=256, height=256):
    ...
    if request_url:
        request_url = OR_CODE_API_LINK_FORMAT.format(
            request_url, gtin, tpl, width, height
        )
